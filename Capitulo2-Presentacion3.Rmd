...
* Repaso
* Intervalos de Confianza (Regiones de Confianza)
* Prueba de Hipótesis
    + Decisiones $0-1$
    + Factor de Bayes
    + El veto a los *a priori* imprópios
...
## Spectral image Fig. 2.2.

```{r, warning=FALSE, message=FALSE}
CMBdata <- read_csv("CMBdata", col_names = FALSE)
image_matrix=matrix(CMBdata$X1, nrow = 800, ncol = 800, byrow = TRUE)
image(image_matrix, axes = FALSE, col=grey(seq(0, 1, length = 256)))
```

## CMBdata: Histograma y normal ajustada. Fig. 2.3.

```{r, warning=FALSE}
hist(CMBdata$X1,breaks = 100, freq = FALSE)
normal_fit(CMBdata$X1,"black")
```


## Subventana aleatoria con histograma de diferencias
...
- La obtención/derivación de los intervalos de confianza (o regiones de confianza en una situación más general) está basada en la distribución posterior $\pi(\theta,\mathcal{D})$. 
- Dado que el enfoque Bayesiano trata/procesa al parámetro $\theta$ como una variable aleatoria (se le asigna una distribución), una definición "natural" de la región de confianza sobre $\theta$ es determinar $C(\mathcal{D})$ tal que 
$$
\tag{2.9}
...
- **La grán diferencia, desde una perspectiva tradicional, es que la integración se hace sobre el espacio de parámetros, en lugar del espacio muestral.**
- **La cantidad $1-\alpha$ entonces corresponde a la probabilidad de que un $\theta$ aleatorio pertenece al conjunto $C(\mathcal{D})$, en lugar de la probabilidad de que un conjunto (intervalo) aleatorio contiene el "verdadero" valor del parámetro $\theta$.**

## Intervalos de Confianza (**highest posterior density (HPD) region**)

- Dado este cambio en la interpretación de un conjunto de confianza (también llamado conjunto con credibilidad/creible por los Bayesianos - Credible Regions), la determinación del mejor intervalo resulta más fácil que en el sentido clásico: simplemente corresponde al valor de $\theta$ con el mayor valor para la distribución posterior.
$$
\begin{array}{rl}
C_{\alpha}^{\pi}(\mathcal{D})=&\{\theta; \pi(\theta\ |\ \mathcal{D})\ge k_{\alpha}\},\\
C_{\alpha}^{\pi}=&\{\theta; \pi(\theta\ |\ x)\ge k_{\alpha}\}
\end{array}
$$
donde $k_\alpha$ es el cuantíl tal que se cumple la restricción de cobertura (2.9). Esta región es llamada *highest posterior density (HPD) region* (región de mayor densidad posterior).

## Intervalos de Confianza (**Credible Regions**)

$$
C_{\alpha}^{\pi}=\{\theta; \pi(\theta\ |\ x)\ge k_{\alpha}\}
$$

- Las regiones de Mayor Densidad Posterior tienen la mayor probabilidad de contener el parámetro $\theta$ para un volumen dado.

![](figures/HPD2.png)

## *A priori*  de Jeffreys (Jeffreys prior prior)

$$
\pi(\mu, \sigma^2)=1/\sigma^2
$$
distribución posterior de $(\mu, \sigma)$
$$
\begin{array}{rl}
\mu|\sigma,\bar{x},s_x^2\sim& \mathcal{N}\bigg(\bar{x},\frac{\sigma^2}{n}\bigg),\\
\sigma^2|\bar{x},s_x^2\sim&\mathcal{IG}\bigg(\frac{n-1}{2},\frac{s_x^2}{2}\bigg)
\end{array}
$$
entonces
$$
\begin{array}{rl}
\pi(\mu|\bar{x},s_x^2)\propto& \displaystyle \int \omega^{1/2} \exp -\omega\frac{n(\bar{x}-\mu)^2}{2}\omega^{(n-3)/2}\exp\{-\omega \ s_x^2/2\}d\omega\\ 
\propto&[n(\mu-\bar{x})^2+s_x^2]^{-n/2}
\end{array}
$$
distribución t-Student con $n-1$ grados de libertad $[\mathcal{T}_{n-1}]$

## Intervalos de Confianza (**normaldata**)
...
$$
con $n-1=89$ grados de libertad, como se puede ver en (2.7).

## Intervalos de Confianza (**Normal credibel interval**)
...
## Intervalos de Confianza (**Cálculos**)
...
## Intervalos de Confianza (**Ejemplo 2.1.**)
...
## Intervalos de Confianza (**Ejemplo 2.1.**) (Cont.)

Una solución analítica de $\tilde\pi(\mu|\mathcal{D})$ no es posible, sólo numérica, y el cálculo de una cota apropiada $k_\alpha$ requiere un nivel extra de cómputos numéricos para obtener la cobertura correcta. 

## Intervalos de Confianza (**Ejemplo 2.1.**) (Cont.)

![Fig 2.5. Distribución posterior de $\mu$ para una distribución *a priori* $\mathcal{N}(0,10)$ y una región HPD de $95\%$.](figures/HPD2.png)

## Intervalos de Confianza (**Nota de cierre**)
...
- Decidir la validez de algunas premisas o restricciones sobre el parámetro $\theta$ es una gran parte del trabajo del estadístico. 
$$
H_0:\theta \in \Theta_0\ \text{ versus }\ H_1:\theta\notin\Theta_0
$$

- Como el resultado del proceso de decisión es claro (*clearcut*), binario, aceptar (representado por 1) o rechazar (representado por 0), la construcción y la evaluación de los procedimientos es crucial.
$$
\mathcal{D}=\{0,1\}
$$

- Mientras que la solución Bayesiana es formalmente muy cercana a un estadístico de la razón de la verosimilitud, sus valores numéricos frecuentemente difieren fuertemente de las soluciones clásicas.
...
## Decisiones $0-1$ (**Función de pérdida 0-1**)
...
## Decisiones $0-1$ (**Función de pérdida 0-1**) (Cont.)
...
\end{array}
$$
donde $x$ denota los datos $\mathcal{D}$ disponibles. 

## Decisiones $0-1$ (**Función de pérdida 0-1**) (Cont.)

Bajo la función de pérdida $0-1$, el criterio de decisión de Bayes (estimador) asociado con la distribución *a priori* $\pi$ es
$$
\delta^\pi(x)=\Bigg\{
\begin{array}{lr}
...
## Decisiones $0-1$ (**Función de pérdida 0-1 ponderada**)

Una generalización de la pérdida mencionada anteriormente es la penalización de los errores de una manera diferente cuando la hipótesis nula es verdadera en lugar de cuando es falsa a través de la función de pérdida $0-1$ ponderada ($a_0,a_1>0$), es decir, Pérdida ponderada $(a_0-a_1)$
$$
...
## Decisiones $0-1$ (**Ejercicio 2.18. - Solución**)
...
## Decisiones $0-1$ (**Ejercicio 2.19.**)

Cuando $\theta\in\{\theta_0,\theta_1\}$, muestre que el procedimiento Bayesiano sólo depende de la razón/ratio $(1-\varrho_0)f_{\theta_1}/\varrho_0f_{\theta_0}(x)$, donde $\varrho_0$ es el peso *a priori* sobre $\theta_0$.

## Decisiones $0-1$ (**Ejercicio 2.19. - Solución**)
...
## Decisiones $0-1$ (**Falta de Robustez**)
...
## Decisiones $0-1$ (**Falta de Robustez**) (Cont.)
...
$$
x\le-\frac{\sigma^2}{\tau^2}\xi - \bigg(1+ \frac{\sigma^2}{\tau^2} \bigg)z_{a_0,a_1}\omega
$$
...
Los procedimientos de Prueba de Hipótesis depende de $P^\pi(\theta\in\Theta_0|x)$ o alternativamente del *Factor de Bayes*, en la ausencia de parámetros $a_0$ y $a_1$ de la función de pérdida.




## Factor de Bayes (**Escala de Evidencia de Jeffreys**)
...
## Factor de Bayes (**Escala de Evidencia de Jeffreys**) (Cont.)
...
## Factor de Bayes (**Reparametrización**)

Alternativamente, se puede notar (perceived) como una razón de verosimilitud Bayesiana dado que si $\pi_0$ y $\pi_1$ son distribuciones *a priori* bajo $H_0$ y $H_1$, respectivamente y si $P^\pi(\theta\in\Theta_0)=P^\pi(\theta\in\Theta_1)=\frac{1}{2}$, $B_{10}^\pi$, entonces dado un *a priori*
$$
...
## Factor de Bayes (**Hipótesis nula puntual**)
...
## Factor de Bayes (**Hipótesis nula puntual**) (Cont.)
...
## Factor de Bayes (**Nota**)

- Note que esta modificación tiene sentido tanto desde el punto de vista de la información como operativo. Si $H_0:\theta=\theta_0$, el hecho de que la hipótesis es puesta a prueba implica que $\theta=\theta_0$ es una posibilidad y trae información *a priori* adicional sobre el parámetro $\theta$. 
- Además, si se prueba $H_0$ y es aceptada, esto significa, en la mayoría de los casos, que el modelo (reducido) bajo $H_0$ se utilizará en lugar del modelo (completo) considerado anteriormente. 
- Entonces, una distribución *a priori* bajo el modelo reducido debe estar disponible para inferencias posteriores potenciales. (Obviamente, el hecho de que esta inferencia posterior depende de la selección de la hipótesis nula $H_0$ también se debería tomar en consideración.)

## Factor de Bayes (**Masa de Dirac**)
...
Para $x\sim\mathcal{N}(\mu,\sigma^2)$ y $\mu\sim\mathcal{N}(\xi,\tau^2)$, considere la prueba $H_0:\mu=0$. Podemos escoger $\xi$ igual a $0$ si no contamos con información *a priori* adicional. Entonces el *Factor de Bayes* es la razón de las marginales bajo ambas hipótesis, $\mu=0$ y $\mu\ne 0$,
$$
\begin{array}{ll}
...
$$
es la probabilidad posterior de $H_0$. 

## Factor de Bayes (**Tabla 2.1**)

La Tabla 2.1 ofrece una idea de los valores de la probabilidad posterior cuando la cantidad normalizada $x/\sigma$ varía. Esta probabilidad posterior obviamente depende de la selección de la variancia *a priori* $\tau^2$: La dependencia es en realidad muy severa, como veremos más adelante con la paradoja de *Jeffreys Lindley*.
...
## Factor de Bayes (**Figura 2.6.**)

A continuación la **Figura 2.6.** con los Datos **normaldata**: Rango del *Factor de Bayes* $B_{10}^\pi$ cuando $\tau$ va de $10^{-4}$ a 10. (El eje $x$ está en escala logarítmica)
...
## El veto a los *a priori* imprópios (Cont.)
...
## El veto a los *a priori* imprópios (**Nota**)
...
## El veto a los *a priori* imprópios (**Paradoja de Jeffrey**)
...
## El veto a los *a priori* imprópios (**CMBdata**)
...
## El veto a los *a priori* imprópios (**Ejercicio 2.21.**)
...
Calcule el valor de la integrar en el denominador de $B_{10}^\pi$ previo.

## El veto a los *a priori* imprópios (**Ejercicio 2.21. - Solución**)
...
\begin{array}{ll}
\displaystyle \int[(\mu-\bar{x})^2+(\mu-\bar{y})^2+S^2]^{-n}d\mu\\
=\displaystyle \int \Bigg(2[\mu- \big(\frac{\bar{x}+\bar{y}}{2}\big)]^2+\frac{(\bar{x}-\bar{y})^2}{2}+S^2\Bigg)^{-n}d\mu\\
=\vdots\\
