...
La función de perdida se define como $L(\theta, \hat{\theta})=\|\theta, \hat{\theta}\|^2$, entonces tenemos:
$$
\begin{array}{rl}
...
Por ser $f_\theta$ una función de densidad de la familia exponencial tenemos:
...
Observemos que $\frac{\partial (\theta R(y))}{\partial \theta}=R(y)$, asi:
$$
\begin{array}{rl}
...
Observemos que $\frac{\partial (e^{\theta R(y)-\psi(\theta))}}{\partial \theta}=e^{\theta R(y)-\psi(\theta)}\frac{\partial (\theta R(y)-\psi(\theta))}{\partial \theta}$ asi:
$$
\begin{array}{rl}
...
Asi tenemos que $\xi(y_1,...,y_n)=\sum_{i=1}^n R(y_i)+\xi$ y $\lambda(y_1,...,y_n)=n+\lambda$
...
\end{array}
$$
donde $h(x)=\frac{e^{-\frac{x^2}{2}}}{\sqrt{2\pi}}$, $\psi(\theta)=\frac{\mu^2}{2}$, $R(x)=x$ y $\theta=\mu$. Asi tenemos que la priori puede escribirse como:
$$
\begin{array}{rl}
...
Veamos que esto implica que la priori conjugada de $\mu$ es tambi?n una normal $\mathcal{N}(\lambda^{-1}\xi,\lambda^{-1})$
$$
\begin{array}{rl}
...
\end{array}
$$
Esto es una normal con media $\frac{\xi}{\lambda}$ y varianza $\frac{1}{\lambda}$.

Para calcular la distribuci?n a priori de $\mu$ usaremos el teorema de Bayes, recordemos que estamos suponiendo que $X\sim \mathcal{N}(\mu,1)$ por lo tanto 
$$
\begin{array}{rl}
...
Con lo cual tenemos una distribuci?n normal con media $\frac{x+\xi}{1+\lambda}$ y varianza $\frac{1}{(1+\lambda)}$
...
De lo anterior sabemos que la priori conjugada es $\pi(\mu|\xi,\lambda) = e^{\mu\xi-\lambda\frac{\mu^2}{2}}$ y la verosimilitud de los datos es $L(\mu|\mathcal{D})=\prod_{i=1}^n \frac{1}{\sqrt{2\pi}}e^{-\frac{1}{2}(x_i-\mu)^2}$, sustituyendo este tenemos:
$$
\begin{array}{rl}
...
Entonces tenemos que $\theta=[\beta, \alpha]$, $R(x)=[\gamma\ln(x), -x^\gamma]^t$
$$
\begin{array}{rl}
...
**Muestre que cuando la priori de $\theta=(\mu, \sigma^2)$ es $\mathcal{N}(\xi,\frac{\sigma^2}{\lambda_\mu})\times \mathcal{IG}(\lambda_s,\alpha)$, la marginal de $\mu$ es una distribuci?n *t-Student* $\mathcal{T}(1\lambda_s,\xi,2\alpha)$. Dar la correspondiente marginal a priori de $\sigma^2$. Para una muestra simple iid $\mathcal{D}=(x_1,...x_n)$ de $\mathcal{N}(\mu,\sigma^2)$**
...
Ya que $u^{(A-1)-1}e^{-uB} \frac{A^{B-1}}{\Gamma(B-1)}$ es la funci?n de densidad de una distribuci?n Gamma con par?metros $A-1$ y $B$
$$
\begin{array}{rl}
\pi(\mu) & \propto B^{-(A-1)} \\
\pi(\mu) & \propto B^{1-A} \\
